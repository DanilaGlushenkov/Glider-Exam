...
def parse_markdown_questions(markdown_path: str | Path) -> QuestionBank:
    ...
    for line_number, raw_line in enumerate(lines, start=1):
        ...
        if h3_match:
            ...
            current_source_short = _source_name_from_h3(source_value) if in_vzor_source else None
            continue
        ...
        if option_match:
            if current_number is None:
                parse_warnings.append(
                    f"Line {line_number}: option outside question context in source {current_source}"
                )
                continue
    ...
    if not current_h2:
        parse_warnings.append("No H2 categories found in markdown input")
